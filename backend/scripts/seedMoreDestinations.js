...
const additionalDestinationsData = [
  {
    name: "Bali",
  ...
  },
  {
    ...
    category: "cultural",
    images: [
      "https://images.unsplash.com/photo-1552832230-c0197047c8d6",
...
const seedMoreDestinations = async () => {
  try {
    console.log('🌱 Starting additional destination seeding process...');
    
    // Connect to database
    await connectDB();
    
    // Insert additional destinations (don't clear existing ones)
    console.log('📍 Inserting additional destinations...');
    const insertedDestinations = await Destination.insertMany(additionalDestinationsData);
    
    console.log(`✅ Successfully seeded ${insertedDestinations.length} additional destinations:`);
    insertedDestinations.forEach((dest, index) => {
      console.log(`   ${index + 1}. ${dest.name}, ${dest.country} (${dest.category})`);
    });
    
    // Get total count of all destinations
    ...
    console.log(`   • New destinations added: ${insertedDestinations.length}`);
    console.log(`   • Categories represented: ${[...new Set(additionalDestinationsData.map(d => d.category))].join(', ')}`);
    console.log(`   • New countries added: ${[...new Set(additionalDestinationsData.map(d => d.country))].length}`);
    
  } catch (error) {
    console.error('❌ Error seeding additional destinations:', error.message);
    process.exit(1);
  } finally {
